...
export async function searchDirections(source: string, destination: string) {
    ...
    const API_URL = 'http://backend_app:3000';

    
  const [sourceResponse, destinationResponse] = await Promise.all([
    fetch(`${API_URL}/places?text=${source}`),
    fetch(`${API_URL}/places?text=${destination}`),
  ]);
  ...
  const directionsResponse = await fetch(
    `http://backend_app:3000/directions?originId=${placeSourceId}&destinationId=${placeDestinationId}`,
    {
